...
std::string GetIpForCache() {
  ...
  for (const net::NetworkInterface& network : networks)
    ip += net::IPAddressToString(network.address);
...
NetworkLocationProvider::PositionCache::PositionCache() {
  ...
  if (chrome::GetDefaultUserDataDirectory(&path)) {
    ...
    if (base::PathExists(path)) {
      ...
      scoped_ptr<base::ListValue> geolocations = base::ListValue::From(std::move(root));
      if (!geolocations)
        return;
...
NetworkLocationProvider::PositionCache::~PositionCache() {
  ...
  base::FilePath path;
  if (chrome::GetDefaultUserDataDirectory(&path)) {
    scoped_ptr<base::ListValue> geolocations(new base::ListValue());
